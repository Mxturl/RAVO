# RAVO Review v0.4.0 多轮编排一期迭代规格书
...
RAVO Review v0.4.0 要把“可配置轮数”升级为“真实多轮评审编排”。
...
如果没有跨轮综合、挑战 brief、复审输入隔离、收敛规则和逐轮证据，`rounds=3` 只是参数变多，产品价值接近 0。v0.4.0 的成功标准是：用户配置 1、2、3 轮时，review 的行为、输入、产物和验收证据都能看出实质差异。
