...
- `docs/ravo-review-v0.3.2-multi-round-spec-zh.md`
...
- 2026-07-09 关于 PM 验收清单、根因分析多模型评审、自然语言标题、全局知识复用、需求共创、完成后下一步建议、Goal Prompt 缺失规格策略和 Spec 模板的讨论

## 产品定义

RAVO v0.3.2 要把 v0.3.1 的“最小治理闭环”升级为“可被 PM 验收、可被多轮评审挑战、可复用跨项目经验的真实工作流”。

v0.3.1 已经解决了 Review 能真实调用 provider、Knowledge 能沉淀 workspace-local 经验、Acceptance 能按证据约束交付状态的问题。v0.3.2 不再继续扩展泛化治理框架，而是补齐当前明确暴露的产品差距：
...
11. Spec 规格书模板。
12. 完成后下一步建议。
13. README、README_ZH、skill 文档、配置模板、smoke-test、prompt-regression 和 installed plugin cache 同步。
...
详细子规格以 `docs/ravo-review-v0.3.2-multi-round-spec-zh.md` 为准。本总规格只约束跨模块验收口径。
...
- v0.3.2 的脱敏实现级别是敏感词/路径/API key/canary 规则检查加 Agent 明示边界，不宣称完整 DLP。
...
Goal 模式执行 v0.3.2 时，完成条件不是“代码写完”或“三条脚本通过”。

必须全部满足：

- `docs/ravo-v0.3.2-decision-complete-spec-zh.md` 中所有 required 内容完成。
- `docs/ravo-review-v0.3.2-multi-round-spec-zh.md` 中所有 required Review 内容完成。
...
> RAVO v0.3.2 支持 Review 1 到 3 轮真实编排，默认二轮；复杂需求会先进入需求共创；复杂根因可引入多模型评审；Codex 发起验收时会给出 PM 可验收的清单和证据；中高复杂任务可在授权边界内复用项目与全局经验。
