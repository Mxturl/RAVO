...
function main() {
  ...
  const alignmentRef = argValue("--alignment-ref", "").trim();
  const releaseSlice = argValue("--release-slice", "").trim();
  const requirementIds = argValues("--requirement-id");
...
Created: ${now}
${alignmentRef ? `AlignmentRef: \`${alignmentRef}\`\n` : ""}${releaseSlice ? `Release Slice: \`${releaseSlice}\`\n` : ""}${requirementIds.length ? `Requirement Set: ${requirementIds.map((id) => `\`${id}\``).join(", ")}\n` : ""}
...
${alignmentRef ? `- ${alignmentRef}` : "- Record the confirmed Alignment before marking this Spec decision-complete."}

Scope selection method:

${sectionList(argValues("--scope-selection-method"), "For multi-candidate planning, state how candidates were compared; a single hotfix may record its skip reason.")}

Candidate clusters and cost judgment:

| Candidate cluster | Priority | User value | Key dependency | Agent active time | Calendar time | PM involvement | Token range | Uncertainty | Disposition |
|---|---|---|---|---|---|---|---|---|---|
| 待补充 | 待补充 | 待补充 | 待补充 | unknown | unknown | unknown | unknown | 待补充 | 待补充 |

Single product direction:

${sectionList(argValues("--product-direction"), "State the one product direction selected for this Release Slice and why other candidates are not included.")}

Candidate item disposition:

${sectionList(argValues("--candidate-disposition"), "Record include, defer, reject, or clarify for every candidate item.")}

PM confirmation:

${sectionList(argValues("--pm-confirmation"), "Record the PM decision that authorizes this Release Slice.")}
